...
enum { SchemeNorm, SchemeSel, SchemeStat, SchemeLast }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, m->tagset[m->seltags] & 1 << i ? &scheme[SchemeSel] : (occ & 1 << i ? &scheme[SchemeStat] : &scheme[SchemeNorm]));
...
setup(void)
{
	...
	scheme[SchemeStat].border = drw_clr_create(drw, statbordercolor);
	scheme[SchemeStat].bg = drw_clr_create(drw, statbgcolor);
	scheme[SchemeStat].fg = drw_clr_create(drw, statfgcolor);
	/* init bars */
	updatebars();
	updatestatus();
